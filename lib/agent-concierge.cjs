...
const {
  ...
  detectLanguage,
  nonFitReasonArabic,
} = require("./agent-localization.cjs");
...
const NON_FIT_PATTERNS = [
  ["careers", ["job", "career", "cv", "resume", "employment", "hire me", "work for vested", "وظيفة", "وظائف", "سيرة ذاتية", "أبحث عن عمل", "اريد عمل", "أريد العمل لديكم"]],
  ["internships", ["internship", "intern role", "student training", "training course", "training request", "co-op placement", "coop placement", "تدريب", "دورة تدريبية", "برنامج تدريبي"]],
  ["vendor-sales", ["sell you", "sell to vested", "become your supplier", "vendor pitch", "supplier pitch", "software demo", "agency services", "marketing services", "أبيع لكم", "أورد لكم", "أصبح مورداً لديكم", "اصبح موردا لديكم", "عرض خدماتنا عليكم", "تقديم خدماتي لكم"]],
  ["spam-or-seo-schemes", ["backlink", "guest post", "paid link", "link exchange", "bulk email", "casino", "crypto scheme", "باك لينك", "رابط مدفوع", "نشر ضيف", "عملات رقمية"]],
  ["retail-shopping", ["buy a product", "shopping", "retail availability", "online store", "شراء منتج", "متجر إلكتروني", "متجر الكتروني", "بيع بالتجزئة"]],
  ["consumer-visa", ["tourist visa", "family visit visa", "personal visa", "visit visa for me", "تأشيرة سياحية", "زيارة عائلية", "تأشيرة شخصية"]],
  ["unrelated", ["restaurant booking", "hotel booking", "personal concierge", "local errands", "حجز مطعم", "حجز فندق", "خدمات شخصية"]],
];

const SERVICE_TERMS = {
  "market-entry-planning": ["market entry", "entry path", "entry model", "launch plan", "saudi expansion", "دخول السوق", "التوسع في السعودية", "خطة الإطلاق", "خطة الاطلاق"],
  "company-formation-setup": ["formation", "misa", "commercial registration", "saudi entity", "license", "licensing", "تأسيس شركة", "تأسيس كيان", "وزارة الاستثمار", "السجل التجاري", "ترخيص"],
  "managed-local-operations": ["local operations", "managed operations", "administration", "back office", "operating layer", "العمليات المحلية", "إدارة العمليات", "الادارة التشغيلية", "تشغيل الشركة"] ,
  "hr-payroll-saudization": ["hr", "payroll", "hire", "hiring", "gosi", "qiwa", "saudization", "nitaqat", "الموارد البشرية", "الرواتب", "التوظيف", "التأمينات", "قوى", "السعودة", "نطاقات"],
  "finance-vat-zakat-controls": ["finance", "accounting", "vat", "zakat", "zatca", "e-invoicing", "invoice", "المالية", "المحاسبة", "ضريبة القيمة المضافة", "الزكاة", "هيئة الزكاة", "الفوترة الإلكترونية", "الفوترة الالكترونية"],
  "legal-compliance-coordination": ["legal", "compliance", "governance", "contract", "due diligence", "قانوني", "الامتثال", "الحوكمة", "العقود", "العناية الواجبة"],
  "workspace-facilities": ["office", "workspace", "facility", "facilities", "fit-out", "it setup", "مكتب", "مساحة عمل", "مرافق", "تجهيز المكتب"],
  "vendor-registration-procurement-readiness": ["vendor registration", "vendor-registration", "supplier registration", "supplier-registration", "procurement", "tender", "aramco", "pif", "evidence pack", "تسجيل مورد", "تسجيل الموردين", "المشتريات", "مناقصة", "أرامكو", "صندوق الاستثمارات", "حزمة الأدلة"],
};

const AMBIGUOUS_SUPPLIER_TERMS = ["supplier", "vendor", "provider", "مورد", "مورداً", "مزود", "مقدم خدمة"];
const CLIENT_REGISTRATION_TERMS = ["vendor registration", "vendor-registration", "supplier registration", "supplier-registration", "supplier onboarding", "procurement portal", "register with aramco", "register with pif", "تسجيل مورد", "تسجيل شركتنا كمورد", "التسجيل لدى أرامكو", "التسجيل لدى ارامكو", "التسجيل لدى صندوق الاستثمارات", "بوابة المشتريات"];
...
function detectNonFit(text, language = "en") {
  for (const [id, terms] of NON_FIT_PATTERNS) {
    ...
    return {
      ...
      reason: language === "ar"
        ? nonFitReasonArabic(id)
        : rule?.message || "This request is outside Vested KSA's market-entry inquiry scope.",
    };
  }
  return null;
}

function inferSkill(text, requestedSkill, language = "en") {
  if (SKILLS.has(requestedSkill)) return requestedSkill;
  if (detectNonFit(text, language)) return "explain_non_fit_routing";
  if (includesAny(text, ["prepare inquiry", "inquiry draft", "contact vested", "proposal", "request a consultation", "إعداد استفسار", "تجهيز استفسار", "التواصل مع فيستد", "طلب استشارة"])) {
    return "prepare_project_inquiry";
  }
  if (includesAny(text, CLIENT_REGISTRATION_TERMS.concat(["tender", "aramco", "pif", "evidence pack", "مناقصة", "أرامكو", "صندوق الاستثمارات", "حزمة الأدلة"]))) {
    return "prepare_vendor_readiness_plan";
  }
  if (includesAny(text, ["90 day", "90-day", "first three months", "launch brief", "launch plan", "90 يوم", "تسعين يوم", "أول ثلاثة أشهر", "خطة الإطلاق", "خطة الاطلاق"])) {
    return "build_90_day_launch_brief";
  }
  if (includesAny(text, ["misa", "commercial registration", "nitaqat", "saudization", "vat", "zakat", "zatca", "e-invoicing", "payroll requirements", "وزارة الاستثمار", "السجل التجاري", "نطاقات", "السعودة", "ضريبة القيمة المضافة", "الزكاة", "الفوترة الإلكترونية", "متطلبات الرواتب"])) {
    return "identify_misa_hr_tax_requirements";
  }
  if (includesAny(text, ["compare", "entry path", "entry model", "branch", "subsidiary", "entity or", "formation option", "مقارنة", "مسار الدخول", "نموذج الدخول", "فرع", "شركة تابعة", "خيارات التأسيس"])) {
    return "compare_entry_paths";
  }
  if (includesAny(text, ["what does vested", "vested services", "which services", "how can vested", "company overview", "ما خدمات فيستد", "ماذا تقدم فيستد", "خدمات vested", "كيف تساعدنا فيستد"])) {
...
function assessFit(text, language = "en") {
  const nonFit = detectNonFit(text, language);
  if (nonFit) {
    return {
  ...
  if (includesAny(text, AMBIGUOUS_SUPPLIER_TERMS) && !includesAny(text, CLIENT_REGISTRATION_TERMS)) {
    return {
      classification: "maybe_fit",
      confidence: "low",
      route: "recommend_public_resources_first",
      reason: language === "ar"
        ? "لم يتضح ما إذا كانت الشركة تحتاج دعماً لتسجيلها كمورد لدى عميل سعودي أم أنها تعرض خدماتها على Vested KSA. يلزم توضيح المقصود قبل إعداد أي استفسار."
        : "It is unclear whether the company needs help registering as a supplier to a Saudi buyer or is pitching services to Vested KSA. Clarify the intent before preparing an inquiry.",
      matchedSignals: ["ambiguous-supplier-intent"],
      shouldPrepareInquiry: false,
    };
  }
  ...
  if (includesAny(text, ["saudi", "ksa", "riyadh", "kingdom of saudi arabia", "السعودية", "المملكة", "الرياض"])) signals.push("saudi-market-context");
  if (includesAny(text, ["foreign company", "international company", "global company", "expand", "enter saudi", "شركة أجنبية", "شركة اجنبية", "شركة دولية", "شركة عالمية", "التوسع", "دخول السعودية"])) signals.push("international-expansion-context");
  if (includesAny(text, ["vested ksa", "vested services", "what does vested", "فيستد", "خدمات vested"])) signals.push("vested-information-request");
  ...
  if (uniqueSignals.length) {
    return {
      ...
      reason: language === "ar"
        ? "يتطابق الطلب مع نطاق Vested KSA لدخول السوق السعودي أو الجاهزية التشغيلية أو المشتريات."
        : "The request matches Vested KSA's Saudi market-entry, operating-readiness, or procurement scope.",
  ...
  return {
    ...
    route: "recommend_public_resources_first",
    reason: language === "ar"
      ? "لا يتضمن الطلب بعد سياقاً كافياً عن دخول الشركة إلى السوق السعودي لتأكيد الملاءمة."
      : "The request does not yet include enough Saudi market-entry context to confirm fit.",
    matchedSignals: [],
    shouldPrepareInquiry: false,
  };
}
...
function buildSkillContent(skillId, fit, matchedServices, language = "en") {
  if (language === "ar") return arabicSkillContent(skillId, fit, matchedServices);
  const commonRisks = [
...
function buildResponseText(result) {
  if (result.language === "ar") return buildArabicResponseText(result);
  const lines = [
...
function buildConciergeResponse(query, options = {}) {
  ...
  const skillId = inferSkill(text, requestedSkill, language);
  const fit = assessFit(text, language);
  const matchedServices = fit.classification === "not_fit" ? [] : matchServices(text);
  const content = buildSkillContent(skillId, fit, matchedServices, language);
  ...
  const result = {
    ...
    safety: {
      ...
      storesSanitizedDemandSignals: true,

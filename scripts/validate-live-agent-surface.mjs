...
async function checkCanonicalEntry(entry) {
  ...
  if (!htmlResponse.headers.get("content-security-policy")?.includes("default-src 'self'")) {
    fail(`${pagePath}: HTML response missing enforced CSP`);
  }
  ...
  if (!htmlHeadResponse.headers.get("content-security-policy")?.includes("default-src 'self'")) {
    fail(`${pagePath}: HTML HEAD missing enforced CSP`);
  }
